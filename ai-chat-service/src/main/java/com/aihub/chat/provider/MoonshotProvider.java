...
/**
 * Moonshot (Kimi) — OpenAI 兼容协议，支持超长上下文
 */
@Slf4j
@Component
public class MoonshotProvider implements AiProvider {
    ...
    @Value("${aihub.provider.moonshot.api-key:}")
    private String apiKey;

    @Value("${aihub.provider.moonshot.base-url:https://api.moonshot.cn}")
    private String baseUrl;

    public MoonshotProvider(OkHttpClient httpClient, ObjectMapper objectMapper) {
    ...
    public boolean supports(String modelName) {
        return modelName.toLowerCase().contains("moonshot") || modelName.toLowerCase().contains("kimi");
    }
            ...
            try (Response response = httpClient.newCall(httpRequest).execute()) {
                if (!response.isSuccessful()) {
                    return ProviderResponse.builder().success(false)
                            .error("Moonshot返回错误: " + response.code()).build();
                }
                ...
                return ProviderResponse.builder().success(false).error("Moonshot返回空结果").build();
            }
        } catch (IOException e) {
            log.error("Moonshot API 调用异常", e);
            return ProviderResponse.builder().success(false).error("Moonshot调用异常: " + e.getMessage()).build();
        }
    }
        ...
        try {
            ...
            httpClient.newCall(httpRequest).enqueue(new Callback() {
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    if (!response.isSuccessful()) {
                        onError.accept(new RuntimeException("Moonshot返回错误: " + response.code()));
                        return;
                    }
