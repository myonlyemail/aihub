...
public class DoubaoProvider implements AiProvider {
    ...
    @Value("${aihub.provider.doubao.api-key:}")
    private String apiKey;

    @Value("${aihub.provider.doubao.base-url:https://ark.cn-beijing.volces.com}")
    private String baseUrl;

    public DoubaoProvider(OkHttpClient httpClient, ObjectMapper objectMapper) {
    ...
    public boolean supports(String modelName) {
        return modelName.toLowerCase().startsWith("doubao");
    }
            ...
            try (Response response = httpClient.newCall(httpRequest).execute()) {
                if (!response.isSuccessful()) {
                    return ProviderResponse.builder().success(false)
                            .error("Doubao返回错误: " + response.code()).build();
                }
                ...
                return ProviderResponse.builder().success(false).error("Doubao返回空结果").build();
            }
        } catch (IOException e) {
            log.error("Doubao API 调用异常", e);
            return ProviderResponse.builder().success(false).error("Doubao调用异常: " + e.getMessage()).build();
        }
    }
        ...
        try {
            ...
            httpClient.newCall(httpRequest).enqueue(new Callback() {
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    if (!response.isSuccessful()) {
                        onError.accept(new RuntimeException("Doubao返回错误: " + response.code()));
                        return;
                    }
