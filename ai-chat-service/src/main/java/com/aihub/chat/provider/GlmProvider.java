...
public class GlmProvider implements AiProvider {
    ...
    @Value("${aihub.provider.glm.api-key:}")
    private String apiKey;

    @Value("${aihub.provider.glm.base-url:https://open.bigmodel.cn}")
    private String baseUrl;

    public GlmProvider(OkHttpClient httpClient, ObjectMapper objectMapper) {
    ...
    public boolean supports(String modelName) {
        return modelName.toLowerCase().startsWith("glm");
    }
            ...
            try (Response response = httpClient.newCall(httpRequest).execute()) {
                if (!response.isSuccessful()) {
                    return ProviderResponse.builder().success(false)
                            .error("GLM返回错误: " + response.code()).build();
                }
                ...
                return ProviderResponse.builder().success(false).error("GLM返回空结果").build();
            }
        } catch (IOException e) {
            log.error("GLM API 调用异常", e);
            return ProviderResponse.builder().success(false).error("GLM调用异常: " + e.getMessage()).build();
        }
    }
        ...
        try {
            ...
            httpClient.newCall(httpRequest).enqueue(new Callback() {
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    if (!response.isSuccessful()) {
                        onError.accept(new RuntimeException("GLM返回错误: " + response.code()));
                        return;
                    }
