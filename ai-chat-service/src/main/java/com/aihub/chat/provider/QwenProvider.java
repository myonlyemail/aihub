...
public class QwenProvider implements AiProvider {
    ...
    @Value("${aihub.provider.qwen.api-key:}")
    private String apiKey;

    @Value("${aihub.provider.qwen.base-url:https://dashscope.aliyuncs.com}")
    private String baseUrl;

    public QwenProvider(OkHttpClient httpClient, ObjectMapper objectMapper) {
            ...
            try (Response response = httpClient.newCall(httpRequest).execute()) {
                if (!response.isSuccessful()) {
                    return ProviderResponse.builder().success(false)
                            .error("Qwen返回错误: " + response.code()).build();
                }
                ...
                return ProviderResponse.builder().success(false).error("Qwen返回空结果").build();
            }
        } catch (IOException e) {
            log.error("Qwen API 调用异常", e);
            return ProviderResponse.builder().success(false).error("Qwen调用异常: " + e.getMessage()).build();
        }
    }
        ...
        try {
            ...
            httpClient.newCall(httpRequest).enqueue(new Callback() {
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    if (!response.isSuccessful()) {
                        onError.accept(new RuntimeException("Qwen返回错误: " + response.code()));
                        return;
                    }
